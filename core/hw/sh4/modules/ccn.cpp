...
static void CCN_CCR_write(u32 addr, u32 value)
{
	...
	if (temp.ICI) {
		...
		// Phase 7 T1 (fix round 3, DC-boot arming): senkosp's record-walk
		// stub (loader/main.c HANDOFF_SCRATCH 0x8ce94000, senkosp2dreamcast
		// repo) ends with an i-cache invalidate executed from its
		// handoff-scratch page -- the last instruction before the game's
		// own first instruction runs. That's the DC-native equivalent of
		// the Naomi path's first-cart-DMA handoff (cartlog_handoff):
		// strictly after the record walk has written the shim window,
		// strictly before any game write lands, so it can't leak either
		// direction.
		//
		// Fix round 3: the stub doesn't run at HANDOFF_SCRATCH's P1 address
		// -- loader/main.c runs it "through its P2 alias"
		// (P2ADDR(HANDOFF_SCRATCH) = 0x8ce94000 | 0xa0000000 = 0xace94000,
		// shims/include/shim_iface.h P2ADDR), specifically to bypass the
		// icache for freshly-memcpy'd code. Round 2 compared Sh4cntx.pc
		// against the P1 range only, so this trigger structurally never
		// matched (confirmed: rebuilt binary, HANDOFF-DC breadcrumb never
		// printed on a 2nd leg) -- not a dynarec PC-staleness issue, a
		// wrong-mirror address. SH4 P1/P2/P3 are the same physical space
		// mirrored at bits 31:29 = 100/101/110; masking those bits off
		// (& 0x1fffffff) matches the instruction regardless of which
		// mirror it executed through, so this now catches P1, P2, or P3.
