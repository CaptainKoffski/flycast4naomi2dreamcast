...
| `ARENAHW total alloc tex nblk free maxfree nfree` | cartlog | senkosp's KAMUI2 VRAM texture-arena walker (config block 0x8c170eb8, both banks' alloc+free lists, node stride 0x18), run on every STARTRENDER write; emitted only when a running max (total allocated bytes, or texture-class bytes = blocks with flag bit 0x02 clear) increases. Profile-agnostic: measures true peak texture demand on the Naomi profile for DC fix sizing (senkosp Phase 5 fix-scoping). |
| `RNDREG n NEW/HB pb rb span fpu feed olb ispb oll ispl nopbi clip alloc wctl scl bgd bgt nopb wsof pc` | cartlog | Full render/TA buffer-register snapshot at every STARTRENDER write; a line per never-seen layout tuple (FNV-hashed, 512-slot table) + heartbeat every 512 renders. Built for the senkosp round-5 Naomi-arm vs DC-arm PVR config diff (found: DC arm gets a 65x smaller ISP/OPB budget). Includes a one-shot full main-RAM dump to `$FLYCAST_CARTLOG.ram.bin` at render 1500 (poke-site hunt). |
| `PARAMHW tafeed pb` | cartlog | Per-frame raw TA feed byte count (tad context fill at context pop in `rend_start_render`), running max only. Demand side for sizing the DC-arm ISP parameter buffer (senkosp round 5: Naomi-leg peak 0x664e0). |
